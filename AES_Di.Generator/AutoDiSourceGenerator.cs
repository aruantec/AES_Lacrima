...
namespace AES_Di.Generator
{
    ...
    public class AutoDiSourceGenerator : IIncrementalGenerator
    {
        ...
        private static void Execute(Compilation compilation, ImmutableArray<INamedTypeSymbol?> classes, SourceProductionContext context)
        {
            ...
            foreach (var symbol in classes.OfType<INamedTypeSymbol>())
            {
                ...
                if (hasAutoRegister)
                {
                    ...
                    {
                        case "Singleton":
                            sb.AppendLine(Indent(3) + ".SingleInstance()");
                            break;
                        case "Transient":
                            sb.AppendLine(Indent(3) + ".InstancePerDependency()");
                            break;
                        default:
        ...
        private static void GenerateInjectionPartial(SourceProductionContext context, INamedTypeSymbol symbol)
        {
            ...
            var initializeDependenciesModifier = HasBaseInitializeDependencies(symbol) ? "new " : string.Empty;
            ...
            sb.AppendLine(Indent(2) + $"internal {initializeDependenciesModifier}void InitializeDependencies(global::Autofac.IComponentContext context)");
            sb.AppendLine(Indent(2) + "{");
            ...
            // Handle LifeCycle
            sb.AppendLine(Indent(4) + "if (this is IViewModelBase vm) {");
            sb.AppendLine(Indent(5) + "vm.Prepare();");
            sb.AppendLine(Indent(5) + "_ = vm.PrepareAsync();");
        ...
        /// <summary>
        /// Determines whether a base type already provides or will itself
        /// receive an <c>InitializeDependencies</c> method so the generated
        /// member can explicitly hide it.
        /// </summary>
        /// <param name="symbol">Type being generated.</param>
        /// <returns><c>true</c> if a matching base member exists.</returns>
        private static bool HasBaseInitializeDependencies(INamedTypeSymbol symbol)
        {
            for (var baseType = symbol.BaseType; baseType is not null; baseType = baseType.BaseType)
            {
                if (baseType.GetAttributes().Any(a =>
                    a.AttributeClass?.Name == "AutoRegisterAttribute" ||
                    a.AttributeClass?.ToDisplayString() == "AES_Core.DI.AutoRegisterAttribute"))
                {
                    return true;
                }

                foreach (var member in baseType.GetMembers("InitializeDependencies").OfType<IMethodSymbol>())
                {
                    if (member.Parameters.Length != 1)
                        continue;

                    var parameterType = member.Parameters[0].Type.ToDisplayString();
                    if (parameterType == "Autofac.IComponentContext")
                        return true;
                }
            }
        ...
        private static string GetLifetime(INamedTypeSymbol symbol)
        {
            var attr = symbol.GetAttributes().FirstOrDefault(a => a.AttributeClass?.Name == "AutoRegisterAttribute" || a.AttributeClass?.ToDisplayString() == "AES_Core.DI.AutoRegisterAttribute");
            if (attr == null) return "Scoped";
            ...
            var named = attr.NamedArguments.FirstOrDefault(n => n.Key == "Lifetime").Value;
            if (!named.IsNull)
            {
                return named.Value?.ToString() ?? "Scoped";
            }
