...
namespace AES_Controls.Composition
{
    ...
    public class CompositionCarouselControl : ItemsControl, IScaleExclusionRenderTarget
    {
        ...
        {
            if (_pendingVisibleLoad && Bounds.Width > 0 && Bounds.Height > 0 && IsVisible)
        ...
        public void ReloadCoverImages()
        {
            ...
            ScheduleInitialImageLoad();
            ScheduleVisibleCoverImages();
            ScheduleAllMissingCoverImages();
        }
        ...
        private void SetVisualDirectIndexFollow(bool enabled)
        {
            if (_visualDirectIndexFollow == enabled)
                return;

            _visualDirectIndexFollow = enabled;
            _visual?.SendHandlerMessage(new DirectIndexFollowMessage(enabled));
        }

        private bool ShouldUpdateSelectedItemBoundsThisFrame()
        {
        ...
        private void UpdateSelectedItemBounds()
        {
            ...
            {
                if (!BoundsNearlyEqual(_lastPublishedSelectedItemBounds, bounds))
                {
                    _lastPublishedSelectedItemBounds = bounds;
                    SelectedItemBounds = bounds;
                }
            }
            else if (_lastPublishedSelectedItemBounds != default)
            {
                _lastPublishedSelectedItemBounds = default;
                SelectedItemBounds = default;
            }
        ...
        private void ApplyVisualTargetIndex(double clamped)
        {
            ...
            bool directFollow = UsesDirectIndexFollow;
            SetVisualDirectIndexFollow(directFollow);
            if (directFollow)
            {
        ...
        private void PublishSelectedIndex(double index, bool force = false)
        {
            ...
            if (!force && (_isPointerScrolling || _isDragging))
            {
                ...
                int lastRounded = double.IsNaN(_lastPublishedSelectedIndex)
                    ? rounded
                    : (int)Math.Round(_lastPublishedSelectedIndex);
                if (rounded == lastRounded)
                    return;
            }
            else if (!force &&
                     !double.IsNaN(_lastPublishedSelectedIndex) &&
                     Math.Abs(clamped - _lastPublishedSelectedIndex) < SelectedIndexCoalesceDelta)
            {
                return;
            }

            _lastPublishedSelectedIndex = clamped;
            if (Math.Abs(SelectedIndex - clamped) < 0.0001)
                return;

            _suppressSelectedIndexSideEffects = (_isPointerScrolling || _isDragging) && !force;
            try
            {
        ...
        private int IndexAtPoint(Point point)
        {
            ...
            double interactionIndex = IndexForInteraction;
            int hit = HitTest(point, size, interactionIndex);
            if (hit != -1) return hit;
            ...
            double targetFloat = interactionIndex + (relativeX / (itemWidth * 1.5));
        ...
        private int GetDragTargetIndex(Point pointerPoint)
        {
            ...
            int maxIndex = Math.Max(0, _images.Count - 1);
            if (_images.Count == 1 || _draggingIndex < 0)
                return 0;

            var dragCenter = GetDragVisualPoint(pointerPoint);
            double centerX = Bounds.Width / 2.0;
            double pitch = Math.Max(52.0, ItemWidth * ItemScale * ItemSpacing * 0.62);
            double floatSlot = IndexForInteraction + (dragCenter.X - centerX) / pitch;
            floatSlot = Math.Clamp(floatSlot, 0, maxIndex);

            if (double.IsNaN(_dragSlotFloat))
            {
                _dragSlotFloat = floatSlot;
            }
            else
            {
                const double deadZone = 0.4;
                int prevSlot = (int)Math.Round(_dragSlotFloat);
                int nextSlot = (int)Math.Round(floatSlot);
                if (nextSlot > prevSlot && floatSlot < prevSlot + deadZone)
                    floatSlot = _dragSlotFloat;
                else if (nextSlot < prevSlot && floatSlot > prevSlot - deadZone)
                    floatSlot = _dragSlotFloat;
                else
                    _dragSlotFloat = floatSlot;
            }

            return (int)Math.Round(Math.Clamp(_dragSlotFloat, 0, maxIndex));
        }
        ...
        /// <summary>
        /// Edge-only auto-scroll while reorder-dragging. Uses the lifted card position so
        /// holding near an edge scrolls the row without fighting the drop-slot logic.
        /// </summary>
        private void UpdateDragAutoScroll(Point pointerPoint)
        {
            ...
            if (x < zone)
            {
                double t = (zone - x) / zone;
                scrollDelta = -(t * t);
            }
            else if (x > w - zone)
            {
                double t = (x - (w - zone)) / zone;
                scrollDelta = t * t;
            }
            ...
            const double scrollSpeed = 0.11;
            double nextIndex = Math.Clamp(_uiTargetIndex + scrollDelta * scrollSpeed, 0, maxIndex);
            if (Math.Abs(nextIndex - _uiTargetIndex) < 0.0005)
                return;

            PublishSelectedIndex(nextIndex);
        }
        ...
        private void BeginItemDrag(int hit)
        {
            ...
            if (TryGetProjectedItemBounds(hit, IndexForInteraction, out var dragBounds))
            {
        ...
        {
            _initialImageLoadScheduled = false;
            if (_itemsSnapshot.Length == 0 || !IsVisible)
            {
                _pendingVisibleLoad = _itemsSnapshot.Length > 0;
                return;
            }
            ...
            _lastVirtualizationIndex = -1;
            int centerIdx = (int)Math.Clamp(Math.Round(SelectedIndex), 0, _itemsSnapshot.Length - 1);
            ScheduleVirtualization(centerIdx, CancellationToken.None);
            ScheduleVisibleCoverImages();
        }
        ...
        private void ScheduleVirtualization(int centerIdx, CancellationToken ct)
        {
            _ = Task.Run(async () =>
            {
                ...
                {
                    Log.Warn("Carousel image virtualization failed", ex);
                }
            }, ct);
        }
        ...
        private void UpdateVirtualization()
        {
            ...
            if (_virtualizeDebounceTimer == null)
            {
                ...
                _virtualizeDebounceTimer.Tick += (s, e) =>
                {
                    ...
                    ScheduleVirtualization(_lastVirtualizationIndex, _loadCts.Token);
                };
            }
        ...
        private void SyncCarouselViewportMotionState()
        {
            ...
            _viewportMotionTracked = inMotion;
            if (inMotion)
        ...
        protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
        {
            ...
            {
                var newIndex = change.GetNewValue<double>();
                _uiTargetIndex = newIndex;
                _lastPublishedSelectedIndex = newIndex;
                if (!_visualDirectIndexFollow)
                    _visual?.SendHandlerMessage(newIndex);
            ...
            {
                if (change.GetNewValue<bool>() && _itemsSnapshot.Length > 0)
                    ScheduleInitialImageLoad();
            }
        ...
        internal void ImportCoverImagesFrom(CompositionCardGridControl? source)
        {
            ...
            foreach (var (item, _, _) in source.EnumerateDecodedCovers())
            {
        ...
        private IEnumerable<int> BuildCoverLoadOrder()
        {
            ...
            int center = (int)Math.Clamp(Math.Round(SelectedIndex), 0, count - 1);
            yield return center;
        ...
        private void ScheduleVisibleCoverImages()
        {
            ...
            int end = Math.Min(_itemsSnapshot.Length - 1, center + window);
            for (int i = start; i <= end; i++)
        ...
        private void ScheduleCoverImageAtIndex(int index)
        {
            ...
            if (index < _images.Count &&
                ...
                _imageCache.TryGetValue(request.Item, out var cachedImage) &&
                _itemImageSourceKeys.TryGetValue(request.Item, out var cachedKey) &&
                Equals(cachedKey, request.SourceKey))
            {
                if (!ReferenceEquals(_images[index], cachedImage))
                    AssignItemImage(request.Item, index, cachedImage, request.SourceKey);
                return;
            }
            ...
            {
                var fromBitmap = CompositionBitmapHelper.ToSkImage(request.BitmapValue, CachedCarouselImageSize);
                if (fromBitmap != null)
                {
                    var imageToUse = RegisterSharedImage(request.SourceKey, fromBitmap);
                    AssignItemImage(request.Item, request.Index, imageToUse, request.SourceKey);
                    return;
                }
            }
            ...
            _ = Task.Run(async () =>
            {
                ...
                try
                {
                    skImage = await DecodeCoverRequestAsync(request).ConfigureAwait(false);
                    await Dispatcher.UIThread.InvokeAsync(() =>
                    {
                        if (!IsCurrentSnapshotItem(request.Item, request.Index))
                        {
                            if (skImage != null)
                                DisposeImage(skImage);
                            return;
                        }

                        if (skImage == null || IsPlaceholderImage(skImage))
                        {
                            SetLoading(index, ShouldShowLoadingSpinner(index));
                            return;
                        }

                        var imageToUse = RegisterSharedImage(request.SourceKey, skImage);
                        AssignItemImage(request.Item, request.Index, imageToUse, request.SourceKey);
                    }, DispatcherPriority.Render);
                }
                catch (Exception ex)
                {
                    Log.Warn($"Failed to decode carousel cover for index {index}", ex);
                    await Dispatcher.UIThread.InvokeAsync(
                        () => SetLoading(index, ShouldShowLoadingSpinner(index)),
        ...
        private bool TryCreateCoverDecodeRequest(int index, out CoverDecodeRequest request)
        {
            ...
            request = new CoverDecodeRequest(index, item, sourceKey, fileName, bitmapValue, _sectionPlaceholderBitmap);
            return true;
        }
        ...
        private static SKImage? LoadAndResizeStatic(string file, MediaItem? owner)
        {
            try
            {
                if (CompositionMetadataCoverHelper.IsMetadataCachePath(file))
                {
                    ...
                    return bytes == null
                        ? null
                        : CompositionMetadataCoverHelper.LoadCoverFromBytes(
                            bytes,
                            CachedCarouselImageSize,
                            bmp => CreateCarouselImageStatic(bmp, file, owner?.FileName, owner));
                }
                ...
                return CreateCarouselImageStatic(bmp, file, owner?.FileName, owner);
            }
            catch
            {
                return null;
        ...
        private async Task VirtualizeAsync(int centerIdx, CancellationToken ct)
        {
            ...
            foreach (var key in _imageCache.Keys.ToList())
            {
                    ...
                    Math.Abs(cachedIndex - centerIdx) > retainWindow)
                {
                    ...
                    {
                        _images[cachedIndex] = GetPlaceholder();
                        _visual?.SendHandlerMessage(new UpdateImageMessage(cachedIndex, _images[cachedIndex]));
                    }
            ...
            for (int offset = 0; offset <= loadWindow; offset++)
            {
                ...
                if (left >= 0)
                {
                    int capturedLeft = left;
                    await Dispatcher.UIThread.InvokeAsync(
                        () => ScheduleCoverImageAtIndex(capturedLeft),
                        DispatcherPriority.Background);
                }
                ...
                int capturedRight = right;
                await Dispatcher.UIThread.InvokeAsync(
                    () => ScheduleCoverImageAtIndex(capturedRight),
                    DispatcherPriority.Background);
            }
        ...
        private async Task<bool> TryLoadItemAsync(int index, object?[] items, string? bitmapProp, string? fileProp, CancellationToken ct)
        {
            ...
            if (_imageCache.TryGetValue(item, out var cachedImage))
            {
                ...
                if (hasMatchingSource)
                {
                    ...
                    {
                        TouchCacheItem(item);
                        if (index < _images.Count && !ReferenceEquals(_images[index], cachedImage))
                        {
                            await Dispatcher.UIThread.InvokeAsync(() =>
                            {
            ...
            if (realImage != null && realImage.Width > 0 && !IsPlaceholderImage(realImage))
            {
                await Dispatcher.UIThread.InvokeAsync(() =>
                {
                    if (ct.IsCancellationRequested)
        ...
        private void Item_PropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            Dispatcher.UIThread.Post(() =>
            {
                ...
                if (sender == null || (e.PropertyName != bitmapProp && e.PropertyName != fileProp &&
                                       e.PropertyName != nameof(MediaItem.CoverBitmap) &&
                                       e.PropertyName != nameof(MediaItem.LocalCoverPath) &&
                                       e.PropertyName != "CoverFound" &&
                                       e.PropertyName != nameof(MediaItem.IsLoadingCover)))
                    return;
                ...
                {
                    HandleLoadingCoverStateChanged(sender, idx);
                    return;
                }
                ...
                if (e.PropertyName == "CoverFound")
                {
                    ...
                    if (sender is MediaItem { LocalCoverPath: { Length: > 0 } coverPath } && File.Exists(coverPath))
                    {
        ...
        private void CoverReloadDebounceTimer_Tick(object? sender, EventArgs e)
        {
            ...
            int center = (int)Math.Clamp(Math.Round(SelectedIndex), 0, Math.Max(0, _itemsSnapshot.Length - 1));
            var pending = _pendingCoverImageReloads
                .OrderBy(pair => Math.Abs(pair.Value - center))
                .ToArray();
            _pendingCoverImageReloads.Clear();
            _ = Task.Run(() => ReloadCoverImagesBatchAsync(pending));
        }
        ...
        private SKImage? LoadAndResize(string file, MediaItem? owner = null)
        {
            try
            {
                if (CompositionMetadataCoverHelper.IsMetadataCachePath(file))
                {
                    ...
                    return CompositionMetadataCoverHelper.LoadCoverFromBytes(
                        bytes,
                        CachedCarouselImageSize,
                        bmp => CreateCarouselImage(bmp, file, owner?.FileName, owner));
                }
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (delta != 0 || e.Key == Key.Home || e.Key == Key.End)
            {
                ...
                {
                    PublishSelectedIndex(newIndex, force: true);
        ...
        private void FinishDrag(int targetIndex, bool cancel, IPointer? pointer = null)
        {
            ...
            if (targetIndex != _draggingIndex)
            {
                ...
                try
                {
                    ...
                    PublishSelectedIndex(targetIndex, force: true);
                }
        ...
        protected override void OnPointerReleased(PointerReleasedEventArgs e)
        {
            ...
            if (_isDragging)
            {
                _cachedDragTargetIndex = -1;
                _dragSlotFloat = double.NaN;
                int targetIndex = GetDragTargetIndex(e.GetPosition(this));
            ...
            if (_isPressed)
            {
                ...
                PublishSelectedIndex(Math.Clamp(Math.Round(projectedIndex), 0, Math.Max(0, _images.Count - 1)), force: true);
                EndPointerScrollInteraction(forcePublishSelectedIndex: false);
        ...
        private int HitTestOverlayButtons(Point p, int i, Vector2 size)
        {
            ...
            float currentIndex = (float)IndexForInteraction;
            var center = new Vector2(size.X / 2, (float)(size.Y / 2 + VerticalOffset));
            float diff = (float)(i - currentIndex);
            float absDiff = Math.Abs(diff);
