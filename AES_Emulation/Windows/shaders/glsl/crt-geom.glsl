...
if (INV == 1.0){ mul_res = inv_gamma(mul_res,pwr);} 
    ...
    // Saturation
    float luma_ui = dot(mul_res, vec3(0.299, 0.587, 0.114));
    mul_res = mix(vec3(luma_ui), mul_res, uSaturation);
    
    // Tint
    mul_res *= uColorTint.rgb;

// Color the texel.
    output_dummy _OUT;
    _OUT._color = vec4(mul_res, uColorTint.a);
