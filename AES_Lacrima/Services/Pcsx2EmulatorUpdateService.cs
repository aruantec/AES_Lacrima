...
public sealed record Pcsx2UpdateState(
    string Repository,
...
[AutoRegister]
public partial class Pcsx2EmulatorUpdateService
{
    private const string Repository = "https://github.com/pcsx2/pcsx2";
    private const string ReleasesApiEndpoint = "https://api.github.com/repos/PCSX2/pcsx2/releases?per_page=100";
    private const string CacheKey = "github:PCSX2/pcsx2";
    private const string CacheFileName = "pcsx2-releases-cache.json";
    private const string InstalledVersionMarkerFileName = "pcsx2_version.txt";
    private static readonly ILog Log = AES_Core.Logging.LogHelper.For<Pcsx2EmulatorUpdateService>();
    ...
    public async Task<Pcsx2UpdateState> GetUpdateInfoAsync(
        string sectionKey,
        string sectionTitle,
        string? launcherPath,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            Log.Warn("Failed to fetch PCSX2 update info; returning local status only.", ex);
            return new Pcsx2UpdateState(
                Repository,
                currentVersion,
                null,
                false,
                Array.Empty<string>(),
                $"Failed to check PCSX2 updates: {ex.Message}",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
    ...
    public async Task<Pcsx2UpdateState> DownloadOrUpdateAsync(
        string sectionKey,
        string sectionTitle,
        string? launcherPath,
        bool includePrereleases,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (releases.Count == 0)
            {
                ...
                return new Pcsx2UpdateState(Repository, GetInstalledVersion(emulatorDirectory, noReleaseLauncherPath), null, false, Array.Empty<string>(), "No PCSX2 releases found.", emulatorDirectory, updateDirectory, noReleaseLauncherPath);
            }
            ...
            if (targetRelease == null)
            {
                ...
                return new Pcsx2UpdateState(Repository, GetInstalledVersion(emulatorDirectory, unresolvedVersionLauncherPath), releases[0].Tag, false, releases.Select(static r => r.Tag).Take(12).ToList(), $"Version '{requestedVersion}' was not found.", emulatorDirectory, updateDirectory, unresolvedVersionLauncherPath);
            }
            ...
            return new Pcsx2UpdateState(
                ...
                updateAvailable,
                versions,
                $"PCSX2 {targetRelease.Tag} downloaded and updated.",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
        ...
        {
            Log.Error("PCSX2 update failed.", ex);
            var (emulatorDirectory, updateDirectory) = EnsureDirectories(sectionKey, sectionTitle);
            try
            {
                PrepareUpdateDirectory(updateDirectory);
            ...
            return new Pcsx2UpdateState(
                ...
                false,
                Array.Empty<string>(),
                $"PCSX2 download/update failed: {ex.Message}",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
    ...
    private static (string EmulatorDirectory, string UpdateDirectory) EnsureDirectories(string sectionKey, string sectionTitle)
    {
        ...
        var emulatorDirectory = Path.Combine(ApplicationPaths.EmulatorsDirectory, safeSectionKey, "PCSX2");
    ...
    private async Task<IReadOnlyList<ReleaseInfo>> GetReleasesAsync(bool includePrereleases, bool forceRefresh, CancellationToken cancellationToken)
    {
        ...
        {
            Log.Warn("Rate limit reached for PCSX2 updates; using cached releases.");
            json = cache!.ReleasesJson;
        }
        else
        {
    ...
    private static string? ResolveLauncherPath(string? launcherPath, string emulatorDirectory)
    {
        if (OperatingSystem.IsWindows())
        {
            var prioritized = new[] { "pcsx2-qt.exe", "pcsx2-qtx64-avx2.exe", "pcsx2-qtx64.exe", "pcsx2.exe" };
        ...
        if (OperatingSystem.IsLinux())
        {
            var appImageCandidate = Directory.EnumerateFiles(emulatorDirectory, "*.AppImage", SearchOption.AllDirectories)
                .FirstOrDefault(path => Path.GetFileName(path).Contains("pcsx2", StringComparison.OrdinalIgnoreCase));
        ...
        var candidates = Directory.EnumerateFiles(emulatorDirectory, "*", SearchOption.AllDirectories)
            .Where(static path => Path.GetFileName(path).Contains("pcsx2", StringComparison.OrdinalIgnoreCase))
            .ToList();
