...
public sealed record XemuUpdateState(
    string Repository,
...
[AutoRegister]
public partial class XemuEmulatorUpdateService
{
    private const string Repository = "https://github.com/xemu-project/xemu";
    private const string ReleasesApiEndpoint = "https://api.github.com/repos/xemu-project/xemu/releases?per_page=100";
    private const string CacheKey = "github:xemu-project/xemu";
    private const string CacheFileName = "xemu-releases-cache.json";
    private const string InstalledVersionMarkerFileName = "xemu_version.txt";
    private static readonly ILog Log = AES_Core.Logging.LogHelper.For<XemuEmulatorUpdateService>();
    ...
    public async Task<XemuUpdateState> GetUpdateInfoAsync(
        string sectionKey,
        string sectionTitle,
        string? launcherPath,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var status = updateAvailable
                ? $"New xemu version available: {latest}"
                : string.IsNullOrWhiteSpace(currentVersion)
                    ? "xemu is not installed in this section yet."
                    : $"xemu is up to date ({currentVersion}).";

            if (includePrereleases)
                status += " (Including pre-releases)";

            return new XemuUpdateState(
                Repository,
                currentVersion,
                latest,
                updateAvailable,
        ...
        {
            Log.Warn("Failed to fetch xemu update info; returning local status only.", ex);
            return new XemuUpdateState(
                Repository,
                currentVersion,
                null,
                false,
                Array.Empty<string>(),
                $"Failed to check xemu updates: {ex.Message}",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
    ...
    public async Task<XemuUpdateState> DownloadOrUpdateAsync(
        string sectionKey,
        string sectionTitle,
        string? launcherPath,
        bool includePrereleases,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (releases.Count == 0)
            {
                ...
                return new XemuUpdateState(Repository, GetInstalledVersion(emulatorDirectory, noReleaseLauncherPath), null, false, Array.Empty<string>(), "No xemu releases found.", emulatorDirectory, updateDirectory, noReleaseLauncherPath);
            }
            ...
            if (targetRelease == null)
            {
                ...
                return new XemuUpdateState(Repository, GetInstalledVersion(emulatorDirectory, unresolvedVersionLauncherPath), releases[0].Tag, false, releases.Select(static r => r.Tag).Take(10).ToList(), $"Version '{requestedVersion}' was not found.", emulatorDirectory, updateDirectory, unresolvedVersionLauncherPath);
            }
            ...
            if (selectedAsset == null)
            {
                ...
                return new XemuUpdateState(Repository, GetInstalledVersion(emulatorDirectory, missingAssetLauncherPath), releases[0].Tag, false, releases.Select(static r => r.Tag).Take(10).ToList(), "No compatible xemu asset found for this OS.", emulatorDirectory, updateDirectory, missingAssetLauncherPath);
            }
            ...
            return new XemuUpdateState(
                ...
                updateAvailable,
                versions,
                $"xemu {targetRelease.Tag} downloaded and updated.",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
        ...
        {
            Log.Error("xemu update failed.", ex);
            var (emulatorDirectory, updateDirectory) = EnsureDirectories(sectionKey, sectionTitle);
            try
            {
                PrepareUpdateDirectory(updateDirectory);
            ...
            return new XemuUpdateState(
                ...
                false,
                Array.Empty<string>(),
                $"xemu download/update failed: {ex.Message}",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
    ...
    private static (string EmulatorDirectory, string UpdateDirectory) EnsureDirectories(string sectionKey, string sectionTitle)
    {
        ...
        var emulatorDirectory = Path.Combine(ApplicationPaths.EmulatorsDirectory, safeSectionKey, "Xemu");
    ...
    private async Task<IReadOnlyList<ReleaseInfo>> GetReleasesAsync(bool includePrereleases, bool forceRefresh, CancellationToken cancellationToken)
    {
        ...
        Client.DefaultRequestHeaders.UserAgent.ParseAdd("AES_Lacrima-XemuUpdater/1.0");
        ...
        {
            Log.Warn("Rate limit reached for xemu updates; using cached releases.");
            json = cache!.ReleasesJson;
        }
        else
        {
    ...
    private static ReleaseAsset? SelectAssetForPlatform(IReadOnlyList<ReleaseAsset> assets)
    {
        ...
        if (OperatingSystem.IsWindows())
        {
            return assets.FirstOrDefault(asset =>
                       asset.Name.Equals("xemu-win-x86_64-release.zip", StringComparison.OrdinalIgnoreCase))
                   ?? assets.FirstOrDefault(asset =>
                       ...
                       !asset.Name.Contains("pdb", StringComparison.OrdinalIgnoreCase))
                   ?? assets.FirstOrDefault(asset =>
                       asset.Name.Contains("win", StringComparison.OrdinalIgnoreCase) &&
                       asset.Name.EndsWith(".zip", StringComparison.OrdinalIgnoreCase) &&
                       !asset.Name.Contains("dbg", StringComparison.OrdinalIgnoreCase) &&
        ...
        if (OperatingSystem.IsMacOS())
        {
            return assets.FirstOrDefault(asset =>
                       ...
                       asset.Name.EndsWith(".zip", StringComparison.OrdinalIgnoreCase) &&
                       !asset.Name.Contains("unsigned", StringComparison.OrdinalIgnoreCase) &&
                       !asset.Name.Contains("dbg", StringComparison.OrdinalIgnoreCase));
        }
    ...
    private static string? ResolveLauncherPath(string? launcherPath, string emulatorDirectory)
    {
        if (OperatingSystem.IsWindows())
        {
            var candidate = Directory.EnumerateFiles(emulatorDirectory, "xemu.exe", SearchOption.AllDirectories).FirstOrDefault();
        ...
        {
            var appImage = Directory.EnumerateFiles(emulatorDirectory, "xemu*.AppImage", SearchOption.AllDirectories).FirstOrDefault();
        ...
        var candidates = Directory.EnumerateFiles(emulatorDirectory, "*", SearchOption.AllDirectories)
            .Where(static path =>
            {
                ...
                return fileName.Contains("xemu", StringComparison.OrdinalIgnoreCase);
            })
            .ToList();
