...
public partial class EdenEmulatorUpdateService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var updateAvailable = !string.IsNullOrWhiteSpace(latest) &&
                                  !string.IsNullOrWhiteSpace(currentVersion) &&
                                  !VersionsEquivalent(currentVersion, latest);
    ...
    private async Task<IReadOnlyList<EdenRelease>> GetReleasesAsync(RepoResolution repository, bool forceRefresh, CancellationToken cancellationToken)
    {
    ...
    private static IReadOnlyList<EdenRelease> ParseReleases(string json)
    {
        ...
        if (!isGitHubFormat)
        {
            return ParseForgejoReleases(root);
        }
    ...
    private static IReadOnlyList<EdenRelease> ParseForgejoReleases(JsonArray root)
    {
        var results = new List<EdenRelease>();
        foreach (var node in root)
        {
