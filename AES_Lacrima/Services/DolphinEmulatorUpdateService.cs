...
{
    private const string Repository = "https://emulationking.com/nintendo/gamecube/emulator/dolphin";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var status = updateAvailable
                ...
                : string.IsNullOrWhiteSpace(currentVersion)
                    ? "Dolphin is not installed in this section yet."
                    : $"Dolphin is up to date ({currentVersion}).";
        ...
        {
            Log.Warn("Failed to fetch Dolphin update info; returning local status only.", ex);
            return new DolphinUpdateState(
                Repository,
                currentVersion,
                null,
                false,
                Array.Empty<string>(),
                $"Failed to check Dolphin updates: {ex.Message}",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
    ...
    public async Task<DolphinUpdateState> DownloadOrUpdateAsync(
        string sectionKey,
        string sectionTitle,
        string? launcherPath,
        bool includePrereleases,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (releases.Count == 0)
            {
                ...
                return new DolphinUpdateState(Repository, GetInstalledVersion(emulatorDirectory, noReleaseLauncherPath), null, false, Array.Empty<string>(), "No Dolphin versions found from update sources.", emulatorDirectory, updateDirectory, noReleaseLauncherPath);
            }
            ...
            if (targetRelease == null)
            {
                ...
                return new DolphinUpdateState(Repository, GetInstalledVersion(emulatorDirectory, unresolvedVersionLauncherPath), releases[0].Tag, false, releases.Select(static r => r.Tag).Take(12).ToList(), $"Version '{requestedVersion}' was not found.", emulatorDirectory, updateDirectory, unresolvedVersionLauncherPath);
            }
            ...
            if (selectedAsset == null)
            {
                ...
                return new DolphinUpdateState(Repository, GetInstalledVersion(emulatorDirectory, missingAssetLauncherPath), releases[0].Tag, false, releases.Select(static r => r.Tag).Take(12).ToList(), "No compatible Dolphin asset found for this OS.", emulatorDirectory, updateDirectory, missingAssetLauncherPath);
            }
            ...
            return new DolphinUpdateState(
                ...
                updateAvailable,
                versions,
                $"Dolphin {targetRelease.Tag} downloaded and updated.",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
        ...
        {
            Log.Error("Dolphin update failed.", ex);
            var (emulatorDirectory, updateDirectory) = EnsureDirectories(sectionKey, sectionTitle);
            try
            {
                PrepareUpdateDirectory(updateDirectory);
            ...
            return new DolphinUpdateState(
                ...
                false,
                Array.Empty<string>(),
                $"Dolphin download/update failed: {ex.Message}",
                emulatorDirectory,
                updateDirectory,
                resolvedLauncherPath);
        }
    ...
    private static (string EmulatorDirectory, string UpdateDirectory) EnsureDirectories(string sectionKey, string sectionTitle)
    {
        ...
        var emulatorDirectory = Path.Combine(ApplicationPaths.EmulatorsDirectory, safeSectionKey, "Dolphin");
    ...
    private async Task<IReadOnlyList<ReleaseInfo>> GetReleasesAsync(bool includePrereleases, bool forceRefresh, CancellationToken cancellationToken)
    {
        ...
        {
            Log.Warn("Rate limit reached for Dolphin updates; using cached releases.");
            json = cache!.ReleasesJson;
        }
        else
        {
    ...
    private async Task<IReadOnlyList<ReleaseInfo>> GetEmulationKingReleasesAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            using var response = await Client.GetAsync(EmulationKingDolphinPageEndpoint, cancellationToken).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
        ...
        {
            Log.Warn("Failed to fetch Dolphin versions from EmulationKing.", ex);
            return Array.Empty<ReleaseInfo>();
        }
    }
    ...
    private static IReadOnlyList<ReleaseInfo> ParseEmulationKingReleases(string html)
    {
        ...
        foreach (Match match in anchorRegex.Matches(html))
        {
            ...
            Uri downloadUri;
            if (!Uri.TryCreate(hrefRaw, UriKind.Absolute, out downloadUri!))
            {
                if (!Uri.TryCreate(new Uri(EmulationKingDolphinPageEndpoint), hrefRaw, out downloadUri!))
                    continue;
            }
            ...
            var versionMatch = Regex.Match(text, "Dolphin\\((?<v>[^\\)]+)\\)", RegexOptions.IgnoreCase);
            if (!versionMatch.Success)
                continue;

            var tag = versionMatch.Groups["v"].Value.Trim();
            if (string.IsNullOrWhiteSpace(tag))
                continue;

            releases.Add(new ReleaseInfo(tag, false, null, new[] { new ReleaseAsset(fileName, downloadUri.ToString()) }));
        }
    ...
    private async Task<IReadOnlyList<ReleaseInfo>> GetEmucrReleasesAsync(CancellationToken cancellationToken)
    {
        try
        {
            Client.DefaultRequestHeaders.UserAgent.Clear();
            ...
            using var response = await Client.GetAsync(EmucrDolphinFeedEndpoint, cancellationToken).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();

            var json = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
            var stubs = ParseEmucrReleaseStubs(json);
            if (stubs.Count == 0)
                return Array.Empty<ReleaseInfo>();

            var releases = new List<ReleaseInfo>();
            foreach (var stub in stubs.Take(12))
            {
                try
                {
                    using var postResponse = await Client.GetAsync(stub.PostUrl, cancellationToken).ConfigureAwait(false);
                    postResponse.EnsureSuccessStatusCode();
                    var postHtml = await postResponse.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
                    var assets = ParseEmucrWindowsAssetsFromPostHtml(postHtml);
                    if (assets.Count == 0)
                        continue;

                    releases.Add(new ReleaseInfo(stub.Tag, false, stub.PublishedAt, assets));
                }
                catch (Exception ex)
                {
                    Log.Warn($"Failed to fetch EmuCR Dolphin post: {stub.PostUrl}", ex);
                }
            }
        ...
        {
            Log.Warn("Failed to fetch Dolphin versions from EmuCR feed.", ex);
            return Array.Empty<ReleaseInfo>();
        }
    }

    private static IReadOnlyList<EmucrReleaseStub> ParseEmucrReleaseStubs(string json)
    {
        if (string.IsNullOrWhiteSpace(json))
            return Array.Empty<EmucrReleaseStub>();
        ...
        var entries = root?["feed"]?["entry"] as JsonArray;
        if (entries == null)
            return Array.Empty<EmucrReleaseStub>();

        var releases = new List<EmucrReleaseStub>();
        foreach (var entryNode in entries)
        {
            if (entryNode is not JsonObject entry)
                continue;

            var title = entry["title"]?["$t"]?.GetValue<string>()?.Trim();
            if (string.IsNullOrWhiteSpace(title) || !title.Contains("Dolphin", StringComparison.OrdinalIgnoreCase))
                continue;

            var tag = ExtractEmucrTag(title);
            if (string.IsNullOrWhiteSpace(tag))
                continue;

            var postUrl = string.Empty;
            if (entry["link"] is JsonArray links)
            {
                foreach (var linkNode in links)
                {
                    if (linkNode is not JsonObject linkObj)
                        continue;

                    var rel = linkObj["rel"]?.GetValue<string>();
                    if (!string.Equals(rel, "alternate", StringComparison.OrdinalIgnoreCase))
                        continue;

                    postUrl = linkObj["href"]?.GetValue<string>()?.Trim() ?? string.Empty;
            ...
            var published = entry["published"]?["$t"]?.GetValue<string>();
            if (DateTimeOffset.TryParse(published, out var parsedPublished))
                publishedAt = parsedPublished;

            releases.Add(new EmucrReleaseStub(tag, publishedAt, postUrl));
        }
    ...
    private static string? ExtractEmucrTag(string title)
    {
        var match = Regex.Match(title, "Dolphin(?:\\s+Git)?\\s+(?<v>[0-9A-Za-z.-]+)", RegexOptions.IgnoreCase);
        if (match.Success)
    ...
    private static IReadOnlyList<ReleaseAsset> ParseEmucrWindowsAssetsFromPostHtml(string? html)
    {
        if (string.IsNullOrWhiteSpace(html))
        ...
        foreach (Match match in anchorRegex.Matches(html))
        {
            ...
            var linkText = Regex.Replace(match.Groups["text"].Value, "<[^>]+>", " ");
            linkText = WebUtility.HtmlDecode(linkText).Trim();
            var combinedText = $"{linkText} {href}";

            var isWindows = combinedText.Contains("windows", StringComparison.OrdinalIgnoreCase) ||
                            combinedText.Contains("x64", StringComparison.OrdinalIgnoreCase) ||
                            combinedText.Contains("win", StringComparison.OrdinalIgnoreCase);
            if (!isWindows)
                continue;
            ...
            assets.Add(new ReleaseAsset(fileName, href));
        }

        return assets;
    }

    private async Task<IReadOnlyList<ReleaseInfo>> GetSelectorReleasesAsync(bool includePrereleases, CancellationToken cancellationToken)
    {
        try
        {
            var releases = new List<ReleaseInfo>();
        ...
        {
            Log.Warn("Failed to fetch Dolphin selector releases; falling back to GitHub releases.", ex);
            return Array.Empty<ReleaseInfo>();
        }
    }

    private async Task<IReadOnlyList<ReleaseInfo>> FetchSelectorReleasesAsync(string endpoint, bool isPrerelease, CancellationToken cancellationToken)
    {
        Client.DefaultRequestHeaders.UserAgent.Clear();
        Client.DefaultRequestHeaders.UserAgent.ParseAdd("Dolphin Updater/1.0");

        using var response = await Client.GetAsync(endpoint, cancellationToken).ConfigureAwait(false);
        response.EnsureSuccessStatusCode();
        var json = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
        return ParseSelectorReleases(json, isPrerelease);
    }

    private static IReadOnlyList<ReleaseInfo> ParseSelectorReleases(string json, bool isPrerelease)
    {
        if (string.IsNullOrWhiteSpace(json))
            return Array.Empty<ReleaseInfo>();
        ...
        {
            var version = FirstString(obj, "tag_name", "tag", "version", "name", "build", "label");
            if (string.IsNullOrWhiteSpace(version))
                continue;

            var assets = ParseSelectorAssets(obj);
            if (assets.Count == 0)
                continue;

            DateTimeOffset? publishedAt = null;
            var publishedRaw = FirstString(obj, "published_at", "created_at", "updated_at", "date", "timestamp");
            if (DateTimeOffset.TryParse(publishedRaw, out var parsed))
                publishedAt = parsed;

            releases.Add(new ReleaseInfo(version.Trim(), isPrerelease, publishedAt, assets));
        }

        return releases;
    }

    private static IReadOnlyList<ReleaseAsset> ParseSelectorAssets(JsonObject obj)
    {
        var assets = new List<ReleaseAsset>();

        if (obj["assets"] is JsonArray assetsArray)
        {
            foreach (var item in assetsArray)
            {
                if (item is not JsonObject assetObj)
                    continue;

                var name = FirstString(assetObj, "name", "filename");
                var url = FirstString(assetObj, "browser_download_url", "download_url", "url", "download", "link");
                if (!string.IsNullOrWhiteSpace(url))
                    assets.Add(new ReleaseAsset(string.IsNullOrWhiteSpace(name) ? Path.GetFileName(new Uri(url).AbsolutePath) : name!, url));
            }
        }
        ...
        var directUrl = FirstString(obj, "browser_download_url", "download_url", "url", "download", "link", "file", "archive");
        if (!string.IsNullOrWhiteSpace(directUrl))
        {
            var fileName = Path.GetFileName(new Uri(directUrl).AbsolutePath);
            if (!string.IsNullOrWhiteSpace(fileName))
                assets.Add(new ReleaseAsset(fileName, directUrl));
        }
    ...
    private static ReleaseAsset? SelectAssetForPlatform(IReadOnlyList<ReleaseAsset> assets)
    {
        ...
        if (OperatingSystem.IsMacOS())
        {
            return assets.FirstOrDefault(asset =>
                       asset.Name.Contains("mac", StringComparison.OrdinalIgnoreCase) &&
                       (asset.Name.EndsWith(".dmg", StringComparison.OrdinalIgnoreCase) || asset.Name.EndsWith(".tar.xz", StringComparison.OrdinalIgnoreCase) || asset.Name.EndsWith(".zip", StringComparison.OrdinalIgnoreCase)));
        }
    ...
    private static string? ResolveLauncherPath(string? launcherPath, string emulatorDirectory)
    {
        ...
        if (OperatingSystem.IsLinux())
        {
            var appImageCandidate = Directory.EnumerateFiles(emulatorDirectory, "*.AppImage", SearchOption.AllDirectories)
                .FirstOrDefault(path => Path.GetFileName(path).Contains("dolphin", StringComparison.OrdinalIgnoreCase));
        ...
        var candidates = Directory.EnumerateFiles(emulatorDirectory, "*", SearchOption.AllDirectories)
            .Where(static path => Path.GetFileName(path).Contains("dolphin", StringComparison.OrdinalIgnoreCase))
            .ToList();
