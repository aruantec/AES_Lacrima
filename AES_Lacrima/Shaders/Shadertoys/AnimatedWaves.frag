...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 p = uv - 0.5;
    p.x *= res.x / res.y;
    vec3 vscodeBg = vec3(0.1176, 0.1176, 0.1176); // VS Code-like dark gray (#1e1e1e)
    ...
    float band1 = bandMask(q.y, wave1, 0.26, 0.12);
    float band2 = bandMask(q.y, wave2, 0.21, 0.11);
    float band3 = bandMask(q.y, wave3, 0.30, 0.13);
    float band4 = bandMask(q.y, wave4, 0.34, 0.16);
    ...
    float gasVisibility = max(smoke * 0.95, stream * 0.90);
    gasVisibility = max(gasVisibility, highlight * 0.85);
    float topDark = smoothstep(0.52, 1.0, uv.y);
    float bgDarkPull = (0.52 + topDark * 0.26) * (1.0 - smoothstep(0.08, 0.62, gasVisibility));
    color = mix(color, vscodeBg, bgDarkPull);
    vec3 gasBoostTint = mix(coolBand, warmBand, smoke1);
    color += gasBoostTint * (0.10 * gasVisibility);

    float motionDetail = max(max(band1, band2), max(band3, band4));
    motionDetail = max(motionDetail, smoke * 0.9);
    motionDetail = max(motionDetail, stream * 0.8);
    motionDetail = max(motionDetail, highlight * 0.7);
    motionDetail = max(motionDetail, horizonGlow * 0.45);
    float fxVisibility = smoothstep(0.06, 0.72, motionDetail);
    color = mix(vscodeBg, color, 0.20 + 0.80 * fxVisibility);
