...
vec3 getDynamicColor(vec3 base, vec3 accent, float t) {
    float pulse = 0.5 + 0.5 * sin(t * 0.28);
    float drift = 0.5 + 0.5 * sin(t * 0.11 + 1.7);
    vec3 mixed = mix(base, accent, 0.18 + 0.10 * pulse);
    return sat(mixed * (0.84 + 0.16 * drift));
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // u_primary/u_secondary now come from a host-side 2-second
    // transition that fades current color -> gray -> new color.
    vec3 dynamicHue = getDynamicColor(u_primary, u_secondary, iTime);
    float breathing = 0.46;
