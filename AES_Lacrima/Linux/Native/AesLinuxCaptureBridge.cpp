...
typedef struct
{
    ...
    Window proxy_window;
    int proxy_window_taskbar_hidden;
    int last_proxy_x;
    int last_proxy_y;
    int last_proxy_w;
    int last_proxy_h;
...
static pthread_mutex_t g_x_error_mutex = PTHREAD_MUTEX_INITIALIZER;

static int IgnoreBadWindowXErrorHandler(Display*, XErrorEvent*)
{
    return 0;
}

static void RunWithIgnoredXErrors(Display* display, const std::function<void()>& action)
{
    if (!action)
        return;

    pthread_mutex_lock(&g_x_error_mutex);
    XErrorHandler previousHandler = XSetErrorHandler(IgnoreBadWindowXErrorHandler);
    ...
    pthread_mutex_unlock(&g_x_error_mutex);
}
...
static void SetWindowBelow(Display* display, Window window, bool below)
{
    if (!display || window == 0)
        return;
    ...
    if (netWmState == None || wmStateBelow == None)
        return;
    ...
    ev.xclient.message_type = netWmState;
    ev.xclient.format = 32;
    ev.xclient.data.l[0] = below ? 1 : 0;
    ev.xclient.data.l[1] = static_cast<long>(wmStateBelow);
...
static void HideProxyWindowFromTaskbarIfNeeded(LinuxCapture* cap)
{
    if (!cap || !cap->display || cap->proxy_window == 0)
        return;

    pthread_mutex_lock(&cap->mutex);
    if (!cap->proxy_window_taskbar_hidden)
    {
        SetWindowSkipTaskbar(cap->display, cap->proxy_window, true);
        cap->proxy_window_taskbar_hidden = 1;
        XFlush(cap->display);
        LogNative("proxy window taskbar hidden after PipeWire frame processing started");
    }
...
static pid_t GetWindowPid(Display* display, Window window)
{
    ...
    RunWithIgnoredXErrors(display, [&]() {
        ...
        {
            uint32_t rawPid = *reinterpret_cast<uint32_t*>(prop);
            pidValue = static_cast<pid_t>(rawPid);
        }
    });
...
static Window FindWindowByTitle(Display* display, Window root, const char* titleHint)
{
    ...
    RunWithIgnoredXErrors(display, [&]() {
        ...
        for (unsigned int i = 0; i < childCount; i++)
        {
            ...
            found = FindWindowByTitle(display, child, titleHint);
            if (found != 0)
                return;
        }
    });
...
static Window FindWindowByPid(Display* display, Window root, pid_t pid, const char* titleHint, bool requireTitleMatch)
{
    ...
    RunWithIgnoredXErrors(display, [&]() {
            ...
            if (childPid == pid)
        {
            ...
            found = FindWindowByPid(display, child, pid, titleHint, requireTitleMatch);
            if (found != 0)
                return;
        }
    });
...
static void HideTargetOffscreenIfRequested(LinuxCapture* cap)
{
    ...
    XWindowAttributes attr{};
    bool hasAttributes = false;
    RunWithIgnoredXErrors(cap->display, [&]() {
        ...
        RunWithIgnoredXErrors(cap->display, [&]() {
            ...
            if (XGetWindowAttributes(cap->display, cap->window, &hostAttrs))
            {
                ...
                if (hostAttrs.map_state == IsViewable) {
                    ...
                    SetWindowBelow(cap->display, cap->proxy_window, true);
...
static void PwOnProcess(void* userdata)
{
    ...
    HideProxyWindowFromTaskbarIfNeeded(cap);
}
...
LinuxCapture* aes_linux_capture_create(void* parentHandle)
{
    ...
    // Present it as a utility window first so the WM treats it like a tool surface,
    // while keeping NORMAL as a fallback for portals that filter by app windows.
    Atom window_type = XInternAtom(cap->display, "_NET_WM_WINDOW_TYPE", False);
    Atom window_type_utility = XInternAtom(cap->display, "_NET_WM_WINDOW_TYPE_UTILITY", False);
    Atom window_type_normal = XInternAtom(cap->display, "_NET_WM_WINDOW_TYPE_NORMAL", False);
    Atom window_types[] = { window_type_utility, window_type_normal };
    XChangeProperty(cap->display, cap->proxy_window, window_type, XA_ATOM, 32, PropModeReplace, (unsigned char*)window_types, 2);
...
void aes_linux_capture_destroy(LinuxCapture* cap)
{
    ...
    cap->proxy_window_taskbar_hidden = 0;
