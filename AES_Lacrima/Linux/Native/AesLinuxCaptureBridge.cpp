...
typedef struct
{
    ...
    int hide_target;
    int target_pid;
    char target_hint[128];
    int crop_frame_extents;
    ...
    // Headless composition export (no native X11 embed / EGL present)
    int                      headless;
    pthread_mutex_t          export_mutex;
    struct pw_buffer*        export_buf;
    uint64_t                 export_frame_id;
    uint64_t                 export_next_frame_id;
    int                      export_held;
    int                      export_acquired_fd;
} LinuxCapture;

typedef struct AesLinuxExportFrame
{
    int width;
    int height;
    int stride;
...
static void GetWindowExtents(Display* display, Window window, int* outLeft, int* outRight, int* outTop, int* outBottom)
{
    *outLeft = 0; *outRight = 0; *outTop = 0; *outBottom = 0;
    ...
    auto readCardinalExtents = [&](Atom atom) -> bool
    {
        if (atom == None)
            return false;
        ...
        if (XGetWindowProperty(display, window, atom, 0, 4, False,
        ...
        if (actualType == XA_CARDINAL && actualFormat == 32 && nItems >= 4 && propData)
        {
            ...
            *outLeft   = static_cast<int>(extents[0]);
            *outRight  = static_cast<int>(extents[1]);
            *outTop    = static_cast<int>(extents[2]);
            *outBottom = static_cast<int>(extents[3]);
            ok = (*outLeft > 0 || *outRight > 0 || *outTop > 0 || *outBottom > 0);
        }
    ...
    if (readCardinalExtents(XInternAtom(display, "_NET_FRAME_EXTENTS", True)))
        return;

    readCardinalExtents(XInternAtom(display, "_GTK_FRAME_EXTENTS", True));
}

static void ResolvePipeWireFrameExtents(LinuxCapture* cap, int* outLeft, int* outRight, int* outTop, int* outBottom)
{
    *outLeft = 0;
    *outRight = 0;
    *outTop = 0;
    *outBottom = 0;
    ...
    if (!cap || !cap->crop_frame_extents || !cap->display)
        return;

    Window extentTarget = cap->target;
    if (extentTarget == 0 && cap->target_pid > 0)
        extentTarget = ResolveTargetWindow(cap, cap->target_pid, cap->target_hint[0] != '\0' ? cap->target_hint : nullptr);

    if (extentTarget == 0)
        return;

    Window topLevel = GetTopLevelWindow(cap->display, extentTarget);
    if (!topLevel)
        topLevel = extentTarget;

    GetWindowExtents(cap->display, topLevel, outLeft, outRight, outTop, outBottom);
}
...
static bool EnsureShaderProgram(LinuxCapture* cap)
{
    ...
    const bool loadedCustomFragment = LoadTextFile(cap->shader_path, &customFragment);

    GLuint program = BuildShaderProgram(customFragment);
    if (loadedCustomFragment && customFragment)
        free(customFragment);
...
static void QueueExportBuffer(LinuxCapture* cap, struct pw_buffer* buf);
static bool ParseExportFrame(LinuxCapture* cap, struct pw_buffer* pwbuf, AesLinuxExportFrame* out);
static void ReleaseHeldExportFrameLocked(LinuxCapture* cap);
...
static void* RenderThreadMain(void* arg)
{
    ...
    if (!cap->headless && cap->gl_supported && cap->glx_context && cap->window)
    {
        if (glXMakeCurrent(cap->display, cap->window, cap->glx_context) == True)
            cap->context_is_current = 1;
    }
    ...
    auto EnterPwRenderLoop = [&]() {
        if (cap->headless)
        {
            LogNative("PipeWire: render thread entered headless export mode");
            while (!cap->stop_render_thread && cap->pw_active)
            {
                pthread_mutex_lock(&cap->mutex);
                const int shutdown_requested = cap->pw_shutdown_requested;
                const int reconnect_requested = cap->pw_reconnect_requested;
                pthread_mutex_unlock(&cap->mutex);
                if (shutdown_requested || reconnect_requested)
                    break;
                ...
                pthread_mutex_lock(&cap->mutex);
                SampleSourceFrameMetrics(cap, now);
                cap->initializing = 0;
                pthread_mutex_unlock(&cap->mutex);
                ...
                if (cap->export_buf)
                {
                    if (cap->export_held)
                    {
                        ...
                        QueueExportBuffer(cap, pbuf);
                    }
                    else
                    {
                        struct pw_buffer* old = cap->export_buf;
                        cap->export_buf = pbuf;
                        cap->export_frame_id = ++cap->export_next_frame_id;
                        cap->export_held = 0;
                        cap->export_acquired_fd = -1;
                        pthread_mutex_unlock(&cap->export_mutex);
                        QueueExportBuffer(cap, old);
                    }
                }
                else
                {
    ...
    if (cap->backend_mode == BackendPipeWire && (cap->egl_initialized || cap->headless))
    {
    ...
    // Headless composition: no XComposite host window — only wait for portal/PW.
    if (cap->headless)
    {
        ...
        while (!cap->stop_render_thread)
        {
            ...
            else if (cap->pw_reconnect_requested)
            {
            ...
            if (do_pw_reconnect)
            {
                if (cap->pw_loop)
                {
                    LogNative("PipeWire: headless reconnect — replacing stream for pid=%d", cap->target_pid);
                    if (ReconnectGamescopeDirectPipeWireStream(cap))
                    {
                        EnterPwRenderLoop();
                    ...
                    {
                        LogNative("PipeWire: headless reconnect failed");
                        pthread_mutex_lock(&cap->mutex);
                        cap->initializing = 0;
                        cap->active = 0;
                        SetStatusText(cap, "Gamescope PipeWire reconnect failed");
                        pthread_mutex_unlock(&cap->mutex);
                    }
                }
                continue;
            ...
            if (do_pw_init)
            {
                ...
                LogNative("PipeWire: headless InitPipeWireBackend failed");
                pthread_mutex_lock(&cap->mutex);
                cap->initializing = 0;
                cap->active = 0;
                SetStatusText(cap, "PipeWire portal failed (headless)");
                pthread_mutex_unlock(&cap->mutex);
            }

            if (xfd >= 0)
            {
                pollfd pfd{ xfd, POLLIN, 0 };
                poll(&pfd, 1, 50);
            }
            else
            {
                             ...
                             enum pw_stream_state new_state, const char* error)
{
    ...
    if (new_state == PW_STREAM_STATE_STREAMING)
    {
        ...
        cap->active = 1;
        cap->initializing = 0;
        SetStatusText(cap, cap->headless
            ? "Capturing (PipeWire composition)"
...
static bool InitPipeWireBackend(LinuxCapture* cap)
{
    ...
    if (!cap->headless && !cap->window)
    {
        LogNative("PipeWire: InitPipeWireBackend aborted — no host window");
        return false;
    }
    ...
    {
        LogNative("PipeWire: EGL init skipped (headless composition export)");
    }
    ...
    if (cap->headless)
    {
        SetBackendDetail(cap, "PipeWire headless export");
        SetGpuInfo(cap, "PipeWire DMA-BUF (composition)", "Linux");
        SetStatusText(cap, "PipeWire composition capture active");
    }
    else
    {
    ...
    LogNative("PipeWire: backend initialised (node=%u, headless=%d)", node_id, cap->headless);
    return true;
}
...
static bool ConnectGamescopeDirectPipeWireStream(LinuxCapture* cap, uint32_t node_id, int registry_found)
{
    ...
    LogNative("PipeWire: gamescope direct stream connected (node=%u, pid=%d)",
              connect_id, cap->target_pid);
    return true;
}

static bool ReconnectGamescopeDirectPipeWireStream(LinuxCapture* cap)
{
    if (!cap || !cap->pw_loop || !cap->pw_core)
        return false;
    ...
    cap->export_held = 0;
    cap->export_acquired_fd = -1;
    cap->export_frame_id = 0;
    cap->export_next_frame_id = 0;
    pthread_mutex_unlock(&cap->export_mutex);
    ...
    const bool connected = ConnectGamescopeDirectPipeWireStream(
        cap,
        registry_data.found ? registry_data.node_id : PW_ID_ANY,
        registry_data.found);
    pw_thread_loop_unlock(cap->pw_loop);
    return connected;
}
...
static bool InitGamescopeDirectPipeWireBackend(LinuxCapture* cap)
{
    ...
    const bool ok = ConnectGamescopeDirectPipeWireStream(
        cap,
        registry_data.found ? registry_data.node_id : PW_ID_ANY,
        registry_data.found);
    pw_thread_loop_unlock(cap->pw_loop);
...
static uint32_t SpaFormatToDrmFourcc(uint32_t spaFormat)
{
    switch (spaFormat)
    {
        case SPA_VIDEO_FORMAT_BGRA: return 0x34324742u;
        case SPA_VIDEO_FORMAT_BGRx: return 0x34324758u;
        case SPA_VIDEO_FORMAT_RGBA: return 0x34324152u;
        default:                    return 0x34325241u;
    }
}
...
static void SampleSourceFrameMetrics(LinuxCapture* cap, uint64_t now)
{
    ...
    if (cap->source_last_event_ns != 0 && now > cap->source_last_event_ns)
    {
        const double dt = static_cast<double>(now - cap->source_last_event_ns) / 1000000000.0;
        if (dt > 0.0)
        {
            const double instantFps = std::clamp(1.0 / dt, 1.0, 240.0);
            const double alpha = dt >= 0.025 ? 0.22 : 0.14;
    ...
    cap->fps = StabilizeReportedFps(cap->source_fps);
    cap->frame_time_ms = cap->fps > 0.0 ? 1000.0 / cap->fps : 0.0;
    cap->present_fps = cap->fps;
    cap->present_frame_time_ms = cap->frame_time_ms;
}

static void QueueExportBuffer(LinuxCapture* cap, struct pw_buffer* buf)
{
    if (!cap || !buf || !cap->pw_loop || !cap->pw_stream)
        return;

    pw_thread_loop_lock(cap->pw_loop);
    if (cap->pw_stream)
        pw_stream_queue_buffer(cap->pw_stream, buf);
    pw_thread_loop_unlock(cap->pw_loop);
}

static void ReleaseHeldExportFrameLocked(LinuxCapture* cap)
{
    if (!cap || !cap->export_held)
        return;

    if (cap->export_acquired_fd >= 0)
    {
        close(cap->export_acquired_fd);
        cap->export_acquired_fd = -1;
    }
...
static bool ParseExportFrame(LinuxCapture* cap, struct pw_buffer* pwbuf, AesLinuxExportFrame* out)
{
    ...
    out->width = cap->pw_width;
    out->height = cap->pw_height;
    out->spa_format = cap->pw_spa_format;
    out->drm_fourcc = SpaFormatToDrmFourcc(cap->pw_spa_format);

    struct spa_buffer* spa_buf = pwbuf->buffer;
    if (!spa_buf || spa_buf->n_datas == 0 || cap->pw_width <= 0 || cap->pw_height <= 0)
        return false;

    struct spa_data* d = &spa_buf->datas[0];
    out->stride = d->chunk ? static_cast<int>(d->chunk->stride) : cap->pw_width * 4;
    out->offset = d->chunk ? static_cast<int>(d->chunk->offset) : 0;

    if (d->type == SPA_DATA_DmaBuf)
    {
        const int fd = static_cast<int>(d->fd);
        if (fd < 0)
            return false;
...
static void StopPipeWireBackend(LinuxCapture* cap)
{
    ...
    if (cap->headless)
    {
        pthread_mutex_lock(&cap->export_mutex);
        ReleaseHeldExportFrameLocked(cap);
        // Drop any pending export buffer — the stream is about to be destroyed and
        // re-queueing into a disconnecting stream can crash PipeWire.
        cap->export_buf = nullptr;
        cap->export_held = 0;
        cap->export_acquired_fd = -1;
        pthread_mutex_unlock(&cap->export_mutex);
...
static void RenderPipeWireFrame(LinuxCapture* cap, struct pw_buffer* pwbuf)
{
    ...
    ResolvePipeWireFrameExtents(cap, &extLeft, &extRight, &extTop, &extBottom);
...
LinuxCapture* aes_linux_capture_create(void* parentHandle)
{
    ...
    LogNative("capture create: display opened (portal-capable build)");

    cap->screen = DefaultScreen(cap->display);
    pthread_mutex_init(&cap->mutex, nullptr);
    cap->context_is_current = 0;

    cap->crop_frame_extents = 1;
...
LinuxCapture* aes_linux_capture_create_headless(void)
{
    pthread_once(&g_x11_threads_once, InitX11ThreadsOnce);
    ...
    cap->screen = DefaultScreen(cap->display);
    pthread_mutex_init(&cap->mutex, nullptr);
    pthread_mutex_init(&cap->export_mutex, nullptr);
    pthread_mutex_init(&cap->pw_frame_mutex, nullptr);
    pthread_cond_init(&cap->pw_frame_cond, nullptr);
    cap->export_acquired_fd = -1;
    cap->gl_supported = 0;
    ...
    cap->last_sample_time_ns = MonotonicNowNs();
    SetGpuInfo(cap, "PipeWire DMA-BUF (composition)", "Linux");
    SetStatusText(cap, "Linux composition capture idle");
    LogNative("headless capture create: display opened");

    cap->stop_render_thread = 0;
    cap->render_thread_started = 1;
    pthread_create(&cap->render_thread, nullptr, RenderThreadMain, cap);
    LogNative("headless render thread started");
    return cap;
}
...
void aes_linux_capture_destroy(LinuxCapture* cap)
{
    ...
    LogNative("headless capture destroy: complete");
    free(cap);
}
...
void aes_linux_capture_set_host_size(LinuxCapture* cap, int width, int height)
{
    if (!cap)
        return;
    ...
    if (!cap->display || cap->window == 0)
        return;

    pthread_mutex_lock(&cap->mutex);
    if (cap->cached_host_w == w && cap->cached_host_h == h)
    {
        pthread_mutex_unlock(&cap->mutex);
        return;
    }
    ...
    XMoveResizeWindow(cap->display, cap->window, 0, 0, static_cast<unsigned int>(w), static_cast<unsigned int>(h));
    XFlush(cap->display);
}
...
static void RequestPipeWirePortalLocked(LinuxCapture* cap, const char* reason)
{
    if (!cap || cap->pw_loop)
        return;
    ...
    cap->active = 1;
    cap->initializing = 1;
    SetStatusText(cap, "Waiting for PipeWire portal (pick the gamescope window)...");
    LogNative("PipeWire portal requested (%s); use_pipewire=%d pw_init=%d pw_loop=%p",
              reason ? reason : "unknown",
              cap->use_pipewire,
              cap->pw_init_requested,
              cap->pw_loop);
}
...
void aes_linux_capture_set_target(LinuxCapture* cap, int processId, const char* windowTitleHint)
{
    ...
    if (processId > 0)
        cap->target_pid = processId;
    if (windowTitleHint)
    {
        strncpy(cap->target_hint, windowTitleHint, sizeof(cap->target_hint) - 1);
        cap->target_hint[sizeof(cap->target_hint) - 1] = '\0';
    }
    else
    {
        cap->target_hint[0] = '\0';
    ...
    if (cap->headless)
    {
        if (processId > 0)
        {
            if (cap->pw_loop)
            {
                // Reconnect the PipeWire stream only — keep pw_loop alive. Full loop
                // teardown (pw_thread_loop_stop) from the render thread crashes PipeWire.
                LogNative("headless set_target: reconnect PipeWire stream to pid=%d", processId);
                cap->use_pipewire = 1;
                cap->active = 1;
                cap->initializing = 1;
                cap->pw_shutdown_requested = 0;
                ...
                pthread_cond_broadcast(&cap->pw_frame_cond);
                pthread_mutex_unlock(&cap->pw_frame_mutex);
                cap->pw_reconnect_requested = 1;
                SetStatusText(cap, "Reconnecting to gamescope PipeWire...");
                pthread_mutex_unlock(&cap->mutex);
                return;
            }
    ...
    if (target == 0)
    {
        RequestPipeWirePortalLocked(cap, "set_target_no_x11_window");
        pthread_mutex_unlock(&cap->mutex);
        return;
    }
...
void aes_linux_capture_set_target_window(LinuxCapture* cap, Window target)
{
    ...
    {
        cap->target = target;
        RequestPipeWirePortalLocked(cap, "headless_set_target_window");
        pthread_mutex_unlock(&cap->mutex);
        return;
    }
...
void aes_linux_capture_begin_portal_session(LinuxCapture* cap)
{
    if (!cap)
        return;

    pthread_mutex_lock(&cap->mutex);
    RequestPipeWirePortalLocked(cap, "begin_portal_session");
    pthread_mutex_unlock(&cap->mutex);
}
...
void aes_linux_capture_stop(LinuxCapture* cap)
{
    ...
    if (cap->headless)
    {
        ...
        if (cap->export_held)
        {
            struct pw_buffer* held = cap->export_buf;
            cap->export_buf = nullptr;
            ReleaseHeldExportFrameLocked(cap);
            cap->export_held = 0;
            pthread_mutex_unlock(&cap->export_mutex);
            if (held)
                QueueExportBuffer(cap, held);
        }
        else
        {
            pthread_mutex_unlock(&cap->export_mutex);
...
void aes_linux_capture_set_crop_frame_extents(LinuxCapture* cap, int crop)
{
    if (!cap)
        return;

    pthread_mutex_lock(&cap->mutex);
    const int normalized = crop ? 1 : 0;
    if (cap->crop_frame_extents != normalized)
    {
        cap->crop_frame_extents = normalized;
        cap->gpu_frame_pending = 1;
    }
    pthread_mutex_unlock(&cap->mutex);
}
...
int aes_linux_capture_try_acquire_frame(LinuxCapture* cap, AesLinuxExportFrame* out)
{
    if (!cap || !out || !cap->headless || cap->stop_render_thread)
        return 0;

    memset(out, 0, sizeof(*out));

    pthread_mutex_lock(&cap->export_mutex);
    if (!cap->export_buf || cap->export_held || !cap->pw_active)
    {
        pthread_mutex_unlock(&cap->export_mutex);
        return 0;
    }
    ...
    cap->export_held = 1;
    const uint64_t frameId = cap->export_frame_id;
    struct pw_buffer* buf = cap->export_buf;
    pthread_mutex_unlock(&cap->export_mutex);

    if (!ParseExportFrame(cap, buf, out))
    {
        pthread_mutex_lock(&cap->export_mutex);
        cap->export_held = 0;
        cap->export_acquired_fd = -1;
...
void aes_linux_capture_release_frame(LinuxCapture* cap, uint64_t frame_id)
{
    if (!cap || !cap->headless)
        return;

    pthread_mutex_lock(&cap->export_mutex);
    if (!cap->export_held || cap->export_frame_id != frame_id || !cap->export_buf)
    {
        pthread_mutex_unlock(&cap->export_mutex);
        return;
    }
    ...
    QueueExportBuffer(cap, buf);
}

int aes_linux_capture_copy_held_frame(LinuxCapture* cap, uint64_t frame_id, void* dst, size_t dst_size)
{
    if (!cap || !cap->headless || !dst || dst_size == 0)
        return 0;
    ...
    struct pw_buffer* pwbuf = cap->export_buf;
    pthread_mutex_unlock(&cap->export_mutex);
